			     ...
			     tp_name_t const *tpn)
{
  ...
  // Mark O'Brien changed this function to start the MD5 hash of a new branch
  // value using the topmost VIA branch value of the SIP message whose new VIA
  // header branch value is being calculated. This addresses a deficiency which
  // appears to have been indicated by a comment the original author added to
  // the preamble of this function:
  //   
  //   * XXX - should include HMAC of previous @Via line.

  if (sip->sip_via->v_branch) {
     //printf("\nstateless_branch() incorporating branch value from sip msg: %s\n", sip->sip_via->v_branch);
     //fflush(stdout);
     su_md5_str0update(md5, sip->sip_via->v_branch);
  } else {
    //printf("\nsip->sip_via->v_branch is NULL!\n");
    //fflush(stdout);
  }
  ...
  // Mark O'Brien commented out the contribution to the stateless
  // VIA branch value of ROUTE headers. Constributions by these
  // headers are NOT RECOMMENDED by RFC 3261. It's fine when
  // proxying a request repeat, but the INVITE and CANCEL for
  // the INVITE have a special relationship where the branch
  // value must be identical on the outgoing request. However,
  // it is uncommon for an INVITE request to have ROUTE headers
  // whereas it's quite common for a CANCEL to have ROUTE
  // headers. This was resulting in different VIA branch values!
